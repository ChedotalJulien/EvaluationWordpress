...
if( ! function_exists( 'pluginsimplon_like_dislike_display' ) ):

    function pluginsimplon_like_dislike_display(){
		...
		if( isset( $_POST['allenable'] ) && $_POST['allenable'] == 'allenable' ){
			$show_like_dislike = true;
		...
		if( $twp_be_enable_like_dislike_button  && $show_like_dislike ){
			...
			if( $twp_be_like_icon_layout == 'layout-2' ){
				$like_image = '<img src="'.esc_url( pluginsimplon_URL ).'/assets/icon/smiling-face.svg" />';
				$dlike_image = '<img src="'.esc_url( pluginsimplon_URL ).'/assets/icon/sad-face.svg" />';
			}else{
				$like_image = '<img src="'.esc_url( pluginsimplon_URL ).'/assets/icon/thumbs-up.svg" />';
				$dlike_image = '<img src="'.esc_url( pluginsimplon_URL ).'/assets/icon/thumbs-down.svg" />';
			} ?>

			<div class="twp-like-dislike-button">

				<?php if( pluginsimplon_can_act( get_the_ID(),'twp-post-like' ) ){
					$can_like = 'can-like';
				}else{
					$can_like = 'cant-like';
				} ?>
				...
				<?php if( pluginsimplon_can_act( get_the_ID(),'twp-post-dislike' ) ){
...
pluginsimplon_like_dislike_display(); ?>