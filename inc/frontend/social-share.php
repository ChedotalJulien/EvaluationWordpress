...
if( ! function_exists( 'pluginsimplon_social_share_display' ) ):

    function pluginsimplon_social_share_display($layout){
				        ...
				        case 'linkedin':
					        ...
					        if( $value ){
					        	...
					        	echo '<div class="twp-social-icons twp-social-linkedin">';
						        	$link = $link = "http://www.linkedin.com/shareArticle?mini=true&amp;title=" . esc_html( $title ) . "&amp;url=" . esc_url( $url ); ?>
									<a class="twp-icon-holder" rel="nofollow" <?php if( $twp_be_open_link_type == 'new-tab' ){ ?>target="_blank" <?php } ?> <?php if( $twp_be_open_link_type == 'new-window' ){ ?>onclick="twp_be_popup_new_window( event,'<?php echo esc_url( $link ); ?>'); " <?php } ?> href="<?php echo esc_url( $link ); ?>" >
										<?php 
										if( $layout == 'layout-2' ){
...
pluginsimplon_social_share_display($layout);