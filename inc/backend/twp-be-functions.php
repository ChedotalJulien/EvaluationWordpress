...
if( ! function_exists( 'pluginsimplon_get_ip_address' )):

    // Get IP Address
    function pluginsimplon_get_ip_address(){
...
if( ! function_exists( 'pluginsimplon_can_act' ) ) :

    // Return Ipaddress
    function pluginsimplon_can_act( $post_ID = false,$action_type ) {
        if( empty( $post_ID ) ){
            return false;
        }
        if( $action_type == 'twp-post-dislike' ){
            $like_ip_list = ( $ip = get_post_meta( $post_ID, 'twp_be_ip_address_lists_dislike', true ) ) ? $ip  : array();
            if( ( $like_ip_list == '' ) || ( is_array( $like_ip_list ) && ! in_array( pluginsimplon_get_ip_address(), $like_ip_list ) ) ){
                return true;
            }
        }
        ...
        if( $action_type == 'be-react-1' ){
            ...
            if( ( empty( $like_ip_list ) ) || ( is_array( $like_ip_list ) && ! in_array( pluginsimplon_get_ip_address(), $like_ip_list ) ) ){
                return true;
            }
        }
