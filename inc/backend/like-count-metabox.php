...
add_action( 'add_meta_boxes', 'pluginsimplon_like_dislike_count_metabox' );
if( ! function_exists( 'pluginsimplon_like_dislike_count_metabox' ) ):

    function  pluginsimplon_like_dislike_count_metabox() {
        
        add_meta_box(
            'pluginsimplon_post_like_dislike_count_metabox',
            esc_html__( 'Like/Dislike Count', 'booster-extension' ),
            'pluginsimplon_post_like_count',
            'post', 
            'normal', 
            'high'
        );
...
if( ! function_exists( 'pluginsimplon_post_like_count' ) ):

    /**
     * Callback function for post option.
    */
	function pluginsimplon_post_like_count() {
		global $post;
		wp_nonce_field( basename( __FILE__ ), 'pluginsimplon_like_dislike_meta_nonce' ); ?>
...
if( ! function_exists( 'pluginsimplon_like_dislike_column_count' ) ):

    /**
     * Display custom column
    **/
    function pluginsimplon_like_dislike_column_count( $column, $post_id ) {
...
add_action( 'manage_post_posts_custom_column' , 'pluginsimplon_like_dislike_column_count', 10, 2 );

if( ! function_exists( 'pluginsimplon_like_dislike_count_column_title' ) ):

    /**
     * Add custom column to post list.
    **/
    function pluginsimplon_like_dislike_count_column_title( $columns ) {
        return array_merge( $columns, array( 'like-dislike-count' => esc_html__( 'Like/Dislike', 'booster-extension' ) ) );
    }

endif;
add_filter( 'manage_post_posts_columns' , 'pluginsimplon_like_dislike_count_column_title' );